...
if (Object.keys(orders).length === 0) {
  orders = [];  // نتأكد أنّنا نعمل بمصفوفة إذا كانت فارغة
}
...
// عرض الطلبات في صفحة تتبع الطلب (بدون رقم الجوال)
...
// تحديث حالة الطلب: يُحدد المعامل newStatus لتحديث الحالة حسب المطلوب
...
document.getElementById('orderForm').addEventListener('submit', function(e) {
  ...
  // تُعلام الإدارة عن الطلب الجديد إذا كان مسؤولاً مسجلاً
  notifyAdmin();
...
// دالة إعلام الإدارة (يمكنك استبدال تنبيه بسيط بخدمات إشعارات آنية)
function notifyAdmin() {
  const loggedInUser = getLoggedInUser();
  if (loggedInUser && loggedInUser.role === 'admin') {
    alert("هناك طلب جديد وارد! يرجى مراجعة لوحة الإدارة.");
  }
}
...
// ----- قسم لوحة الإدارة -----
// تعرض لوحة الإدارة كافة طلبات النقل مع تفاصيل رقم الجوال وخيارات إدارة الحالة
